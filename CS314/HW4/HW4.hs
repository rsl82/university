...
matchEmpty (RPlus x) = matchEmpty x
matchEmpty (RAlt x y) = matchEmpty x || matchEmpty y
matchEmpty (RSeq x y) = matchEmpty x && matchEmpty y
matchEmpty _ = False


-- What is difference between REps and RZero
...
firsts REps = []
firsts RZero = []
firsts (RStar x) = firsts x
firsts (RPlus x) = firsts x
firsts (RAlt x y) = firsts x ++ firsts y
firsts (RSeq REps y) = firsts y
firsts (RSeq (RStar x) y) = firsts x ++ firsts y
firsts (RSeq x _) = firsts x
