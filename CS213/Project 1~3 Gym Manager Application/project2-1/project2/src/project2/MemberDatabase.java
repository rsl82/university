...
public class MemberDatabase {
    ...
    public Member getMember(Member member) {
        ...
        Member findMember = this.mlist[find(member)];
    ...
    private void sortByCountry() {
        ...
        for(int i=0;i<this.size-1;i++) {
            for(int j=i+1;j<this.size;j++) {
                if(this.mlist[i].memberLocationGetter().zipcodeGetter().compareTo(this.mlist[j].memberLocationGetter().zipcodeGetter()) > 0) {
    ...
    /**
     * Print method with membership fee.
     */
    public void printWithFee() {
        if(this.size == 0) {
            System.out.println("Member database is empty!");
            return;
        }
        ...
        for(int i=0;i<this.size;i++) {
            Member memberNow = this.mlist[i];
            System.out.println(memberNow.toString()+", Membership fee: $"+String.format("%.2f",memberNow.membershipFee()));
        }
